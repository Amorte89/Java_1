    ...
    private List<ContactData> generateContacts(int count) {
        ...
        for (int i = 0; i < count; i++) {
            groups.add(new ContactData()
                    ...
                    .withHomepage(String.format("testHomepage%s@.ru", i)).withBday("1").withBmonth("April").withByear("1990").withAday("1").withAmouth("January").withAyear("2000").withAddress2(String.format("testAddress2 %s", i))
                    .withNotes(String.format("testNotes %s", i)).withGroup("test 1"));
        }
        return groups;
    }
}
